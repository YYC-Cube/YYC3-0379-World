...
说明：Pruned = 剪枝版模型，显存占用更低，速度更快；微小画质损失，适合快速迭代试seed
权重：DiffSynth-Studio/MiniMax-H3-Pruned（自动拉取pruned剪枝权重）
输出：batch_output_ref2va_pruned（与NF4版本视频隔离）
运行：python h3_pruned_ref2va_batch_resume.py
"""
import torch
import os
