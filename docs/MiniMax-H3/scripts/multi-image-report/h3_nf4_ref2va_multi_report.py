# -*- coding: utf-8 -*-
"""
NF4 Ref2VA｜批量多图 + Markdown报告完整版（含评分栏 + 缺陷标签 + 闭环评估框架）
来源：MiniMax-H3-DiffSynth本地版-提示词模板.md（最终升级整合版）
特性：
✅ 双日志：batch_log.txt（纯文本） + report.md（Markdown可视化表格）
✅ 表格列：参考图 | Seed | 状态 | 文件路径 | 时间 | 评分(1~10) | 缺陷标签
✅ 评分/缺陷标签人工填写；可选标签：变脸/手崩/抖动/口型错位（多标签逗号分隔）
✅ 报告末尾自动追加【闭环评估与迭代指导框架】
✅ 断点续跑：重复运行不会重复添加旧条目，不覆盖手动评分
运行：python h3_nf4_ref2va_multi_report.py
"""
import torch
import os
